...
bool Quadruped<T>::buildModel(FloatingBaseModel<T>& model) {
  ...
  const CoordinateAxis hipKneeAxis =
      (_robotType == RobotType::LEGBOT) ? CoordinateAxis::Y_NEG
                                      : CoordinateAxis::Y;
  ...
  for (int legID = 0; legID < 4; legID++) {
    ...
    if (sideSign < 0) {
      model.addBody(_hipInertia.flipAlongAxis(CoordinateAxis::Y),
                    ...
                    hipKneeAxis, xtreeHip, xtreeHipRotor);
    } else {
      model.addBody(_hipInertia, _hipRotorInertia, _hipGearRatio, bodyID - 1,
                    JointType::Revolute, hipKneeAxis, xtreeHip,
                    xtreeHipRotor);
    }
    ...
    if (sideSign < 0) {
      model.addBody(_kneeInertia.flipAlongAxis(CoordinateAxis::Y),
                    ...
                    hipKneeAxis, xtreeKnee, xtreeKneeRotor);
      ...
      model.addBody(_kneeInertia, _kneeRotorInertia, _kneeGearRatio, bodyID - 1,
                    JointType::Revolute, hipKneeAxis, xtreeKnee,
                    xtreeKneeRotor);
