...
void computeLegJacobianAndPosition(Quadruped<T>& quad, Vec3<T>& q, Mat3<T>* J,
  ...
  if (J) {
    ...
    if (quad._robotType == RobotType::LEGBOT) {
      // LegBot's MJCF thigh/calf hinge convention moves the foot in body-x
      // opposite to the Mini Cheetah analytic leg template.
      J->operator()(0, 1) *= -1;
      J->operator()(0, 2) *= -1;
    }
  }
