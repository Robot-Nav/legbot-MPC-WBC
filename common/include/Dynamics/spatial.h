...
SVec<T> jointMotionSubspace(JointType joint, CoordinateAxis axis) {
  ...
  if (axis == CoordinateAxis::X || axis == CoordinateAxis::X_NEG)
    v(0) = 1;
  else if (axis == CoordinateAxis::Y || axis == CoordinateAxis::Y_NEG)
    v(1) = 1;
  else
    v(2) = 1;
  if (axis == CoordinateAxis::X_NEG || axis == CoordinateAxis::Y_NEG ||
      axis == CoordinateAxis::Z_NEG) {
    v *= -1;
  }
...
Mat6<T> jointXform(JointType joint, CoordinateAxis axis, T q) {
  ...
  if (joint == JointType::Revolute) {
    ...
    if (axis == CoordinateAxis::X || axis == CoordinateAxis::X_NEG)
      ...
      v(2) = q;
    else if (axis == CoordinateAxis::Z_NEG)
      v(2) = q;
    if (axis == CoordinateAxis::X_NEG || axis == CoordinateAxis::Y_NEG ||
        axis == CoordinateAxis::Z_NEG) {
      v *= -1;
