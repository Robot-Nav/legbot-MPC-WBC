...
struct Args {
  ...
  double forward_start = -1.0;
  double forward_gait = -1.0;
  double speed_ramp = 2.0;
  double key3_print_dt = 0.2;
  bool start_prone = false;
  double standup_ramp = 4.0;
  double standup_hold = 1.0;
  double standup_kp = 20.0;
  double standup_kd = 3.0;
  bool standup_dds_target = false;
  bool realtime = false;
  bool viewer = false;
...
void usage(const char* argv0) {
  std::printf(
      ...
      "[--trot-start sec] [--forward-start sec] "
      "[--forward-gait id] [--speed-ramp sec] [--key3-print-dt sec] "
      "[--start-prone] [--standup-ramp sec] [--standup-hold sec] "
      "[--standup-kp gain] [--standup-kd gain] [--standup-dds-target] "
...
Args parseArgs(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (key == "--forward-start") {
      args.forward_start = std::atof(needValue("--forward-start"));
    } else if (key == "--forward-gait") {
      args.forward_gait = std::atof(needValue("--forward-gait"));
    } else if (key == "--speed-ramp") {
      args.speed_ramp = std::atof(needValue("--speed-ramp"));
    } else if (key == "--key3-print-dt") {
      args.key3_print_dt = std::atof(needValue("--key3-print-dt"));
      if (args.key3_print_dt <= 1e-6) args.key3_print_dt = 0.2;
    } else if (key == "--start-prone") {
      args.start_prone = true;
    } else if (key == "--standup-ramp") {
      args.standup_ramp = std::atof(needValue("--standup-ramp"));
      if (args.standup_ramp < 0.0) args.standup_ramp = 0.0;
    } else if (key == "--standup-hold") {
      args.standup_hold = std::atof(needValue("--standup-hold"));
      if (args.standup_hold < 0.0) args.standup_hold = 0.0;
    } else if (key == "--standup-kp") {
      args.standup_kp = std::atof(needValue("--standup-kp"));
      if (args.standup_kp < 0.0) args.standup_kp = 0.0;
    } else if (key == "--standup-kd") {
      args.standup_kd = std::atof(needValue("--standup-kd"));
      if (args.standup_kd < 0.0) args.standup_kd = 0.0;
    } else if (key == "--standup-dds-target") {
      args.standup_dds_target = true;
    } else if (key == "--viewer") {
      args.viewer = true;
...
double speedCommandAtTime(const Args& args, double time) {
  const double ramp_start = args.forward_start >= 0.0
                                ? args.forward_start
                                : (args.trot_start >= 0.0
                                       ? args.trot_start
                                       : args.locomotion_start);
...
const std::array<double, 12>& standQ() {
  static const std::array<double, 12> q = {
...
std::array<double, 12> nominalStandQ(double height) {
  std::array<double, 12> out{};
  for (int leg = 0; leg < 4; ++leg) {
    const auto q = nominalLegQ(kLegs[leg], height);
    for (int joint = 0; joint < 3; ++joint) {
      out[3 * leg + joint] = q[joint];
    }
  }
  return out;
}

const std::array<double, 12>& proneQ() {
  static const std::array<double, 12> q = {
...
std::array<double, 12> blendQ(const std::array<double, 12>& q0,
                              const std::array<double, 12>& q1,
                              double alpha) {
  std::array<double, 12> out{};
  for (int i = 0; i < 12; ++i) {
    out[i] = (1.0 - alpha) * q0[i] + alpha * q1[i];
  }
  return out;
}
...
void resetRobotToQ(const mjModel* model, const JointMap& map, mjData* data,
                   double base_z, const std::array<double, 12>& q) {
  mju_zero(data->qpos, model->nq);
  mju_zero(data->qvel, model->nv);
  data->qpos[map.base_qpos + 0] = 0.0;
  data->qpos[map.base_qpos + 1] = 0.0;
  data->qpos[map.base_qpos + 2] = base_z;
  data->qpos[map.base_qpos + 3] = 1.0;
  data->qpos[map.base_qpos + 4] = 0.0;
  data->qpos[map.base_qpos + 5] = 0.0;
  data->qpos[map.base_qpos + 6] = 0.0;
  ...
  for (int i = 0; i < 12; ++i) {
    data->qpos[map.qpos[i]] = q[i];
  }
}

void applyQOnlyPd(const JointMap& map, const mjData* data,
                  const std::array<double, 12>& q_des,
                  double kp, double kd, mjData* mutable_data) {
  for (int i = 0; i < 12; ++i) {
    const double q = data->qpos[map.qpos[i]];
    const double qd = data->qvel[map.qvel[i]];
    mutable_data->ctrl[map.ctrl[i]] = kp * (q_des[i] - q) - kd * qd;
  }
}
...
std::array<double, 4> footNormalForces(const mjModel* model,
                                       const mjData* data,
                                       const JointMap& map) {
  std::array<double, 4> footFn = {0.0, 0.0, 0.0, 0.0};
  for (int ci = 0; ci < data->ncon; ++ci) {
    const mjContact& contact = data->contact[ci];
    for (int leg = 0; leg < 4; ++leg) {
      if (contact.geom1 == map.foot_geom[leg] ||
...
const char* key3PhaseName(bool locomotionRequested, bool gait0Requested) {
  if (!locomotionRequested) return "PRE_LOCOMOTION";
  return gait0Requested ? "GAIT0_DIRECT_SIM" : "GAIT4_DIRECT_SIM";
}

const char* phaseName(bool locomotionRequested, bool gait0Requested,
                      bool forwardRequested) {
  if (forwardRequested) return "FORWARD_DIRECT_SIM";
  return key3PhaseName(locomotionRequested, gait0Requested);
}

void printKey3DryrunDebug(const mjModel* model, double time,
                          double phaseStartTime, double speed_cmd,
                          int requestedGait, bool locomotionRequested,
                          bool gait0Requested, bool forwardRequested,
                          const mjData* data, const JointMap& map,
                          const SpiCommand& cmd,
                          const std::array<SpineBoard, 4>& spineBoards) {
  const auto fn = footNormalForces(model, data, map);
  const double diagA = fn[0] + fn[3];
  const double diagB = fn[1] + fn[2];
  double maxAbsQdDes = 0.0;
  double maxAbsTauFf = 0.0;
  double maxKp = 0.0;
  double maxKd = 0.0;
  double qErrNorm = 0.0;
  ...
  for (int leg = 0; leg < 4; ++leg) {
    ...
    const double kp[3] = {cmd.kp_abad[leg], cmd.kp_hip[leg],
                          cmd.kp_knee[leg]};
    const double kd[3] = {cmd.kd_abad[leg], cmd.kd_hip[leg],
                          cmd.kd_knee[leg]};
    const double tauFf[3] = {cmd.tau_abad_ff[leg], cmd.tau_hip_ff[leg],
                             cmd.tau_knee_ff[leg]};
    for (int joint = 0; joint < 3; ++joint) {
      qErrNorm += (qDes[joint] - q[joint]) * (qDes[joint] - q[joint]);
      maxAbsQdDes = std::max(maxAbsQdDes, std::abs(qdDes[joint]));
      maxAbsTauFf = std::max(maxAbsTauFf, std::abs(tauFf[joint]));
      maxKp = std::max(maxKp, kp[joint]);
      maxKd = std::max(maxKd, kd[joint]);
    }
  }

  std::printf(
      "[KEY3_DRYRUN_SIM] t=%.3f phase=%s phase_elapsed=%.3f "
      "requested_gait=%d speed_cmd=%.3f base_z=%.5f "
      "Fn=[%.2f %.2f %.2f %.2f] diag_FR_RL=%.2f diag_FL_RR=%.2f "
      "max_qd_des=%.4f max_tau_ff=%.4f max_kp=%.2f max_kd=%.2f "
      "qerr_norm=%.5f flags=[%d %d %d %d]\n",
      time, phaseName(locomotionRequested, gait0Requested, forwardRequested),
      time - phaseStartTime, requestedGait, speed_cmd,
      data->qpos[map.base_qpos + 2], fn[0], fn[1], fn[2], fn[3], diagA,
      diagB, maxAbsQdDes, maxAbsTauFf, maxKp, maxKd, std::sqrt(qErrNorm),
      cmd.flags[0], cmd.flags[1], cmd.flags[2], cmd.flags[3]);

  for (int leg = 0; leg < 4; ++leg) {
    const int i = 3 * leg;
    std::printf(
        "  %s q=[% .4f % .4f % .4f] qDes=[% .4f % .4f % .4f] "
        "qdDes=[% .4f % .4f % .4f] tauFF=[% .3f % .3f % .3f] "
        "ctrl=[% .3f % .3f % .3f]\n",
        kLegs[leg], data->qpos[map.qpos[i + 0]],
        data->qpos[map.qpos[i + 1]], data->qpos[map.qpos[i + 2]],
        cmd.q_des_abad[leg], cmd.q_des_hip[leg], cmd.q_des_knee[leg],
        cmd.qd_des_abad[leg], cmd.qd_des_hip[leg], cmd.qd_des_knee[leg],
        cmd.tau_abad_ff[leg], cmd.tau_hip_ff[leg], cmd.tau_knee_ff[leg],
        spineBoards[leg].torque_out[0], spineBoards[leg].torque_out[1],
        spineBoards[leg].torque_out[2]);
  }
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (args.start_prone) {
      resetRobotToQ(model, map, data, 0.145, proneQ());
    } else {
    ...
    for (int leg = 0; leg < 4; leg++) {
      ...
      spineBoards[leg].resetCommand();
	    ...
	    std::printf("[MuJoCo LegBot Key3 Dryrun] XML: %s\n", args.xml.c_str());
	    std::printf("[MuJoCo LegBot Key3 Dryrun] Start mit_ctrl in another shell: "
	                "./build-legbot-check/user/MIT_Controller/mit_ctrl v s\n");
    const double standupDone =
        args.start_prone ? args.standup_ramp + args.standup_hold : 0.0;
    if (args.start_prone && args.locomotion_start < standupDone + 0.5) {
      const double shift = standupDone + 0.5 - args.locomotion_start;
      args.locomotion_start += shift;
      if (args.trot_start >= 0.0) args.trot_start += shift;
      if (args.forward_start >= 0.0) args.forward_start += shift;
      std::printf(
          "[MuJoCo LegBot Key3 Dryrun] start-prone adjusted key3 schedule by "
          "%.3fs so standup can finish first.\n",
          shift);
    }

    std::printf(
        "[MuJoCo LegBot Key3 Dryrun] sequence: LOCOMOTION+gait4 at %.3fs, "
        "then gait0 at %.3fs, forward ramp at %.3fs. "
        "Target speed is %.3f, forward gait is %.1f.\n",
        args.locomotion_start,
        args.trot_start >= 0.0 ? args.trot_start
                               : args.locomotion_start + 3.0,
        args.forward_start >= 0.0
            ? args.forward_start
            : (args.trot_start >= 0.0 ? args.trot_start
                                      : args.locomotion_start + 3.0),
        args.speed, args.forward_gait);
    if (args.start_prone) {
      std::printf(
          "[MuJoCo LegBot Key3 Dryrun] start-prone enabled: prone q -> stand q "
          "for %.3fs, hold %.3fs, kp=%.2f kd=%.2f target=%s.\n",
          args.standup_ramp, args.standup_hold, args.standup_kp,
          args.standup_kd,
          args.standup_dds_target ? "dds_StandQDds" : "mujoco_nominal");
    }
    ...
    if (args.trot_start < 0.0) args.trot_start = args.locomotion_start + 3.0;
    ...
    double nextKey3Print = 0.0;
    double nextStandupPrint = 0.0;
    double key3PhaseStart = 0.0;
    const auto standupTarget =
        args.standup_dds_target ? standQ() : nominalStandQ(args.height);
    long long stepCount = 0;
    while (data->time < args.duration && (!args.viewer || viewer.alive())) {
      if (!locomotionRequested && data->time >= args.locomotion_start) {
        std::printf("[MuJoCo LegBot Key3 Dryrun] Requesting LOCOMOTION/gait4 at t=%.3f\n",
                    data->time);
        requestControlMode(sharedMemory(), 4);
        requestUserParameter(sharedMemory(), "cmpc_gait", 4);
        locomotionRequested = true;
        key3PhaseStart = data->time;
      }
      if (locomotionRequested && !trotRequested && data->time >= args.trot_start) {
        std::printf("[MuJoCo LegBot Key3 Dryrun] Requesting gait0 stationary trot at t=%.3f\n",
                    data->time);
        requestUserParameter(sharedMemory(), "cmpc_gait", 0);
        trotRequested = true;
        key3PhaseStart = data->time;
      }
      if (trotRequested && !forwardRequested && args.forward_gait >= 0.0 &&
          args.forward_start >= 0.0 && data->time >= args.forward_start) {
        std::printf("[MuJoCo LegBot Key3 Dryrun] Requesting forward gait %.1f at t=%.3f\n",
                    args.forward_gait, data->time);
        requestUserParameter(sharedMemory(), "cmpc_gait", args.forward_gait);
      ...
      if (data->time + 1e-12 >= nextControl) {
        ...
        if (args.start_prone && !locomotionRequested) {
          const double alpha = args.standup_ramp > 1e-9
                                   ? smoothStep(data->time / args.standup_ramp)
                                   : 1.0;
          const auto qTarget = blendQ(proneQ(), standupTarget, alpha);
          applyQOnlyPd(map, data, qTarget, args.standup_kp, args.standup_kd,
                       data);
          if (data->time + 1e-12 >= nextStandupPrint) {
            const char* phase =
                data->time < args.standup_ramp ? "STANDUP_RAMP"
                                                : "STANDUP_HOLD";
            double qerr = 0.0;
            for (int i = 0; i < 12; ++i) {
              const double err = qTarget[i] - data->qpos[map.qpos[i]];
              qerr += err * err;
            }
            std::printf(
                "[KEY3_DRYRUN_SIM] t=%.3f phase=%s alpha=%.3f "
                "base_z=%.5f qerr_norm=%.5f q_target=[%.3f %.3f %.3f ...]\n",
                data->time, phase, alpha, data->qpos[map.base_qpos + 2],
                std::sqrt(qerr), qTarget[0], qTarget[1], qTarget[2]);
            nextStandupPrint += 0.5;
          }
        } else {
          applyControllerOutput(map, sharedMemory().simToRobot.spiData,
                                spineBoards, data);
        }
        if (data->time + 1e-12 >= nextKey3Print) {
          const int requestedGait =
              forwardRequested
                  ? static_cast<int>(std::round(args.forward_gait))
                  : (locomotionRequested ? (trotRequested ? 0 : 4) : -1);
          printKey3DryrunDebug(model, data->time, key3PhaseStart, speedCmd,
                               requestedGait, locomotionRequested,
                               trotRequested, forwardRequested, data, map,
                               sharedMemory().robotToSim.spiCommand,
                               spineBoards);
          nextKey3Print += args.key3_print_dt;
        }
    ...
    sharedMemory().simulatorIsDone();
    std::printf("[MuJoCo LegBot Key3 Dryrun] Finished %.3f s. x=%.3f z=%.3f\n", data->time,
