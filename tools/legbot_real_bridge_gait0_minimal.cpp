...
constexpr double kGait0ShadowWarnAbsTau = 13.0;
constexpr double kGait0ShadowHardAbsTau = 18.0;
constexpr double kGait0ShadowHardAbsQJumpHip = 0.60;
constexpr double kGait0ShadowHardAbsQJumpThigh = 0.75;
constexpr double kGait0ShadowHardAbsQJumpCalf = 0.90;
...
constexpr int kInteractiveFootProbeLeg = 1;
constexpr int kInteractiveFootProbeAxis = 2;
constexpr double kInteractiveFootProbeDelta = 0.01;
constexpr double kInteractiveFootProbeDuration = 2.0;
constexpr double kInteractiveFootProbeRampSeconds = 0.7;
constexpr double kInteractiveFootProbeHoldStart = 0.7;
constexpr double kInteractiveFootProbeHoldEnd = 1.3;
constexpr double kInteractiveFootProbeCartesianKp = 80.0;
constexpr double kInteractiveFootProbeCartesianKd = 4.0;
...
struct FootProbeState {
  bool active = false;
  int leg = kInteractiveFootProbeLeg;
  int axis = kInteractiveFootProbeAxis;
  double delta = kInteractiveFootProbeDelta;
  double duration = kInteractiveFootProbeDuration;
  legbot::Clock::time_point start_time{};
  legbot::Clock::time_point next_print{};
  Vec3<float> p_foot0 = Vec3<float>::Zero();
  std::array<double, 3> q_feedback_at_start{};
};

struct FootProbeDiagnostics {
  std::array<double, 3> p_error{};
  std::array<double, 3> v_error{};
  std::array<double, 3> foot_force{};
  std::array<double, 9> jacobian{};
  std::array<double, 3> tau_cartesian_model{};
  std::array<double, 3> tau_before_joint_scale{};
  std::array<double, 3> tau_after_joint_scale{};
  std::array<double, 3> tau_after_joint_scale_calf05{};
  std::array<double, 3> tau_after_joint_scale_calf10{};
  std::array<double, 3> joint_tau_scale{};
  const char* qdes_source = "unknown";
};

struct SingleLegAlgoProbeState {
  bool active = false;
  bool raw_publish_started = false;
  int leg = 1;
  double duration = 2.0;
  legbot::Clock::time_point start_time{};
  legbot::Clock::time_point raw_publish_start_time{};
  legbot::Clock::time_point next_print{};
};

struct SingleLegAlgoReadiness {
  bool ready = false;
...
double FootProbeSmoothStep(double s) {
  s = std::max(0.0, std::min(1.0, s));
  return s * s * (3.0 - 2.0 * s);
}

double FootProbeAlpha(double elapsed_s) {
  if (elapsed_s < kInteractiveFootProbeHoldStart) {
    return FootProbeSmoothStep(elapsed_s / kInteractiveFootProbeRampSeconds);
  }
  if (elapsed_s < kInteractiveFootProbeHoldEnd) return 1.0;
  if (elapsed_s < kInteractiveFootProbeDuration) {
    return FootProbeSmoothStep(
        1.0 - (elapsed_s - kInteractiveFootProbeHoldEnd) /
                  kInteractiveFootProbeRampSeconds);
  }
  return 0.0;
}

double FootProbeAlpha(double elapsed_s, double duration_s) {
  if (std::abs(duration_s - kInteractiveFootProbeDuration) < 1.e-9) {
    return FootProbeAlpha(elapsed_s);
  }
  if (duration_s <= 0.0 || elapsed_s >= duration_s) return 0.0;
  const double ramp_s = std::min(kInteractiveFootProbeRampSeconds,
                                 std::max(0.0, 0.5 * duration_s));
  const double hold_start = ramp_s;
  const double hold_end = std::max(hold_start, duration_s - ramp_s);
  if (elapsed_s < hold_start) {
    return FootProbeSmoothStep(elapsed_s / std::max(hold_start, 1.e-9));
  }
  if (elapsed_s < hold_end) return 1.0;
  return FootProbeSmoothStep(
      1.0 - (elapsed_s - hold_end) / std::max(duration_s - hold_end, 1.e-9));
}

std::array<double, 9> Mat3TextArray(const Mat3<float>& m) {
  return {static_cast<double>(m(0, 0)), static_cast<double>(m(0, 1)),
          static_cast<double>(m(0, 2)), static_cast<double>(m(1, 0)),
          static_cast<double>(m(1, 1)), static_cast<double>(m(1, 2)),
          static_cast<double>(m(2, 0)), static_cast<double>(m(2, 1)),
          static_cast<double>(m(2, 2))};
}

std::array<double, 3> Vec3dTextArray(const Vec3<float>& v) {
  return {static_cast<double>(v[0]), static_cast<double>(v[1]),
          static_cast<double>(v[2])};
}

FootProbeDiagnostics CaptureFootProbeDiagnostics(
    int leg, const LegController<float>& leg_controller,
    const SpiCommand& spi_command, const legbot::DdsCommandArrays& raw_command,
    double tau_ff_scale, const char* qdes_source) {
  FootProbeDiagnostics diag;
  const auto& data = leg_controller.datas[leg];
  const auto& cmd = leg_controller.commands[leg];
  const Vec3<float> p_error = cmd.pDes - data.p;
  const Vec3<float> v_error = cmd.vDes - data.v;
  const Vec3<float> foot_force =
      cmd.forceFeedForward + cmd.kpCartesian * p_error +
      cmd.kdCartesian * v_error;
  const Vec3<float> tau_cartesian = data.J.transpose() * foot_force;
  Vec3<float> tau_before_scale;
  tau_before_scale << spi_command.tau_abad_ff[leg],
      spi_command.tau_hip_ff[leg], spi_command.tau_knee_ff[leg];

  diag.p_error = Vec3dTextArray(p_error);
  diag.v_error = Vec3dTextArray(v_error);
  diag.foot_force = Vec3dTextArray(foot_force);
  diag.jacobian = Mat3TextArray(data.J);
  diag.tau_cartesian_model = Vec3dTextArray(tau_cartesian);
  diag.tau_before_joint_scale = Vec3dTextArray(tau_before_scale);
  diag.qdes_source = qdes_source ? qdes_source : "unknown";

  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(leg, joint);
    const double scale =
        legbot::GatewayMotorTauScaleForDdsJoint(static_cast<std::size_t>(dds));
    const double scale_calf10 =
        joint == static_cast<int>(legbot::ModelJoint::Calf)
            ? (scale < 0.0 ? -1.0 : 1.0)
            : scale;
    const double tau_model = diag.tau_before_joint_scale[joint];
    diag.joint_tau_scale[joint] = scale;
    diag.tau_after_joint_scale[joint] =
        raw_command.tau[static_cast<std::size_t>(dds)];
    diag.tau_after_joint_scale_calf05[joint] =
        tau_model * scale * tau_ff_scale;
    diag.tau_after_joint_scale_calf10[joint] =
        tau_model * scale_calf10 * tau_ff_scale;
  }
  return diag;
}
...
double Gait0ShadowHardQJumpLimitForDdsJoint(int dds_joint) {
  const int model = legbot::DdsToModelJoint(dds_joint);
  const int joint = model % static_cast<int>(legbot::kNumJointsPerLeg);
  if (joint == static_cast<int>(legbot::ModelJoint::Hip)) {
    return kGait0ShadowHardAbsQJumpHip;
  }
  if (joint == static_cast<int>(legbot::ModelJoint::Thigh)) {
    return kGait0ShadowHardAbsQJumpThigh;
  }
  return kGait0ShadowHardAbsQJumpCalf;
}
...
MitOutputReadiness InspectGait0ShadowReleaseGate(
    ...
    double foot_jump,
    int selected_model_leg = -1) {
  MitOutputReadiness out;
  out.reason = "ok";
  const int first_joint = (selected_model_leg >= 0 && selected_model_leg < 4)
                              ? 0
                              : -1;
  const std::size_t num_checks =
      first_joint >= 0 ? 3 : legbot::kNumJoints;
  for (std::size_t check = 0; check < num_checks; ++check) {
    const int dds =
        first_joint >= 0
            ? legbot::DdsJointIndexFromModel(selected_model_leg,
                                           first_joint +
                                               static_cast<int>(check))
            : static_cast<int>(check);
    const int model = legbot::DdsToModelJoint(dds);
    const std::size_t i = static_cast<std::size_t>(dds);
  ...
  if (!std::isfinite(max_abs_raw_tau) ||
      max_abs_raw_tau >= kGait0ShadowHardAbsTau) {
    std::ostringstream oss;
    oss << "raw_tau_too_large max_abs_raw_tau=" << max_abs_raw_tau
        << " hard_limit=" << kGait0ShadowHardAbsTau
        << " warn_limit=" << kGait0ShadowWarnAbsTau
        << " cmd_tau_limit=" << args.cmd_tau_limit;
    out.reason = oss.str();
...
double DebugMaxFootPDesJumpForLeg(const RobotRunner* runner, int model_leg) {
  const auto* leg_controller = runner ? runner->debugLegController() : nullptr;
  if (!leg_controller || model_leg < 0 || model_leg >= 4) {
    return std::numeric_limits<double>::quiet_NaN();
  }
  double max_jump = 0.;
  const auto& data = leg_controller->datas[model_leg];
  const auto& cmd = leg_controller->commands[model_leg];
  for (int axis = 0; axis < 3; ++axis) {
    max_jump = std::max(
        max_jump,
        std::abs(static_cast<double>(cmd.pDes[axis] - data.p[axis])));
...
double DebugMaxCmpcFootPDesJumpForLeg(const ConvexMPCDebugSnapshot* cmpc,
                                      int model_leg) {
  if (!cmpc || !cmpc->valid || model_leg < 0 || model_leg >= 4) {
    return std::numeric_limits<double>::quiet_NaN();
  }
  double max_jump = 0.;
  for (int axis = 0; axis < 3; ++axis) {
    max_jump = std::max(
        max_jump,
        std::abs(static_cast<double>(cmpc->pFootDes[model_leg][axis] -
                                     cmpc->pFoot[model_leg][axis])));
  }
  return max_jump;
}
...
JointDiffDiagnostic MaxAbsQStepForLeg(
    const legbot::DdsCommandArrays& command,
    const std::array<double, legbot::kNumJoints>& previous_q,
    int model_leg) {
  JointDiffDiagnostic out;
  if (model_leg < 0 || model_leg >= 4) return out;
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(model_leg, joint);
    const std::size_t i = static_cast<std::size_t>(dds);
    if (!std::isfinite(command.q[i]) || !std::isfinite(previous_q[i])) {
      continue;
    }
...
double MaxAbsRawQdForLeg(const legbot::DdsCommandArrays& command,
                         int model_leg) {
  if (model_leg < 0 || model_leg >= 4) {
    return std::numeric_limits<double>::quiet_NaN();
  }
  double max_abs = 0.;
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(model_leg, joint);
    const double qd = command.dq[static_cast<std::size_t>(dds)];
    if (!std::isfinite(qd)) return std::numeric_limits<double>::quiet_NaN();
    max_abs = std::max(max_abs, std::abs(qd));
  }
  return max_abs;
...
double MaxAbsRawTauForLeg(const legbot::DdsCommandArrays& command,
                          int model_leg) {
  if (model_leg < 0 || model_leg >= 4) {
    return std::numeric_limits<double>::quiet_NaN();
  }
  ...
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(model_leg, joint);
    const double tau = command.tau[static_cast<std::size_t>(dds)];
    if (!std::isfinite(tau)) return std::numeric_limits<double>::quiet_NaN();
    max_abs = std::max(max_abs, std::abs(tau));
  }
  return max_abs;
...
void PrintGait0ShadowRawDiagnostics(
    double requested_gait, double phase_elapsed_s,
    const legbot::DdsCommandArrays& raw_command,
    const legbot::LowStateSnapshot& snapshot,
    const RobotRunner* runner, const MIT_Controller* controller) {
  const auto* cmpc = controller ? controller->debugConvexMPC() : nullptr;
  const auto* leg_controller = runner ? runner->debugLegController() : nullptr;
  std::cout << "[GAIT0-SHADOW-RAW]"
  ...
  for (int leg = 0; leg < 4; ++leg) {
    std::array<double, 3> q_feedback{};
    std::array<double, 3> q_des_raw{};
    std::array<double, 3> q_des_error{};
    std::array<double, 3> tau_raw{};
    for (int joint = 0; joint < 3; ++joint) {
      const int dds = legbot::DdsJointIndexFromModel(leg, joint);
      q_feedback[joint] = snapshot.dds_q[static_cast<std::size_t>(dds)];
      q_des_raw[joint] = raw_command.q[static_cast<std::size_t>(dds)];
      q_des_error[joint] = q_des_raw[joint] - q_feedback[joint];
      tau_raw[joint] = raw_command.tau[static_cast<std::size_t>(dds)];
    }

    std::cout << "[GAIT0-SHADOW-RAW-LEG]"
              << " leg=" << leg << "(" << ModelLegName(leg) << ")";
    if (cmpc && cmpc->valid) {
      std::array<double, 3> p_delta{};
      for (int axis = 0; axis < 3; ++axis) {
        p_delta[axis] =
            static_cast<double>(cmpc->pFootDes[leg][axis] -
                                cmpc->pFoot[leg][axis]);
      ...
      std::cout << " pFoot="
                << legbot::ArrayText(Vec3TextArray(cmpc->pFoot[leg]), 5)
                << " pFootDes="
                << legbot::ArrayText(Vec3TextArray(cmpc->pFootDes[leg]), 5)
                << " pFootDelta=" << legbot::ArrayText(p_delta, 5)
                << " vFootDes="
                << legbot::ArrayText(Vec3TextArray(cmpc->vFootDes[leg]), 5)
                << " FrDes="
                << legbot::ArrayText(Vec3TextArray(cmpc->FrDes[leg]), 5);
    ...
    if (leg_controller) {
      const auto& data = leg_controller->datas[leg];
      const auto& cmd = leg_controller->commands[leg];
      std::array<double, 3> p_cmd_delta{};
      for (int axis = 0; axis < 3; ++axis) {
        p_cmd_delta[axis] =
            static_cast<double>(cmd.pDes[axis] - data.p[axis]);
      }
      std::cout << " legDataP="
                << legbot::ArrayText(Vec3TextArray(data.p), 5)
                << " legCmdPDes="
                << legbot::ArrayText(Vec3TextArray(cmd.pDes), 5)
                << " legCmdPDelta="
                << legbot::ArrayText(p_cmd_delta, 5);
    }
    std::cout << " q_feedback=" << legbot::ArrayText(q_feedback, 5)
              << " q_des_raw=" << legbot::ArrayText(q_des_raw, 5)
              << " q_des_raw_minus_feedback="
              << legbot::ArrayText(q_des_error, 5)
              << " tau_raw=" << legbot::ArrayText(tau_raw, 5)
              << "\n";
  }
}
...
const char* FootProbeAxisName(int axis) {
  switch (axis) {
    case 0:
      return "x";
    case 1:
...
void RunGuardedStandup(const legbot::BridgeArgs& args,
                       ...
                       legbot::LoopStats* stats,
                       const legbot::Clock::time_point& stats_t0) {
  const auto standup_start_q = state_adapter->Snapshot().dds_q;
  ...
  std::cout << "[LEGBOT-DDS][STANDUP] start_q="
            << legbot::ArrayText(standup_start_q, 5)
            << " target_q_stand=" << legbot::ArrayText(q_stand, 5) << "\n";
  run_phase("standup_ramp", args.standup_ramp_seconds, standup_start_q,
            q_stand);
}
...
std::string RunFootDeltaProbeTest(
    const legbot::BridgeArgs& args, RobotRunner* runner, SpiData* spi_data,
    SpiCommand* spi_command, VectorNavData* vector_nav_data,
    CheaterState<double>* cheater_state,
    legbot::LegBotDDSStateAdapter* state_adapter,
    legbot::LowCmdPublisher* lowcmd_pub,
    legbot::LegBotDDSCommandAdapter* command_adapter, legbot::LoopStats* stats,
    const legbot::Clock::time_point& stats_t0) {
  if (!runner || !spi_data || !spi_command || !vector_nav_data ||
      !cheater_state || !state_adapter || !lowcmd_pub || !command_adapter) {
    return "foot_delta_probe_missing_dependency";
  }
  auto* leg_controller = runner->debugMutableLegController();
  if (!leg_controller) return "foot_delta_probe_missing_leg_controller";

  int axis = 2;
  if (args.foot_probe_axis == "x") axis = 0;
  if (args.foot_probe_axis == "y") axis = 1;
  Vec3<float> delta = Vec3<float>::Zero();
  delta[axis] = static_cast<float>(args.foot_probe_delta);
  const int probe_leg = args.foot_probe_leg;
  const double period_s = 1. / args.cmd_hz;
  const double joint_hold_kp = std::max(args.standup_kp, 5.0);
  const double joint_hold_kd = std::max(args.standup_kd, 1.0);
  ...
  std::cout << "[FOOT-DELTA-PROBE] start"
            << " leg=" << probe_leg << "(" << ModelLegName(probe_leg) << ")"
            << " axis=" << args.foot_probe_axis
            << " delta=" << args.foot_probe_delta
            << " seconds=" << args.foot_probe_seconds
            << " cart_kp=" << args.foot_probe_kp
            << " cart_kd=" << args.foot_probe_kd
            << " joint_hold_kp=" << joint_hold_kp
            << " joint_hold_kd=" << joint_hold_kd << "\n";

  while (g_running) {
    const auto now = legbot::Clock::now();
    const double elapsed = legbot::SecondsSince(start, now);
    if (elapsed >= args.foot_probe_seconds) break;
    if (state_adapter->IsTimeout()) {
      return "foot_delta_probe_failed:rt_lowstate_timeout";
    }
    ...
    const auto tick_safety = CheckTickAdvances(
        snapshot, args, now, &last_lowstate_tick, &last_tick_change_t);
    if (!tick_safety.ok) {
      return std::string("foot_delta_probe_failed:") + tick_safety.reason;
    }
    legbot::RecordLowStateTiming(snapshot.tick, stats, &last_sample_t,
                               &last_recorded_tick);
    const auto lowstate_safety =
        CheckInteractiveLowState(snapshot, *state_adapter, args, stats);
    if (!lowstate_safety.ok) {
      return std::string("foot_delta_probe_failed:") + lowstate_safety.reason;
    }
    ...
    for (int leg = 0; leg < 4; ++leg) {
      auto& cmd = leg_controller->commands[leg];
      const auto& data = leg_controller->datas[leg];
      cmd.zero();
      cmd.qDes = data.q;
      cmd.qdDes.setZero();
      cmd.kpJoint =
          Mat3<float>::Identity() * static_cast<float>(joint_hold_kp);
      cmd.kdJoint =
          Mat3<float>::Identity() * static_cast<float>(joint_hold_kd);
      if (leg == probe_leg) {
        cmd.pDes = data.p + delta;
        cmd.vDes.setZero();
        cmd.kpCartesian =
            Mat3<float>::Identity() * static_cast<float>(args.foot_probe_kp);
        cmd.kdCartesian =
            Mat3<float>::Identity() * static_cast<float>(args.foot_probe_kd);
      }
    }
    leg_controller->updateCommand(spi_command);
    ...
    if (now >= next_print) {
      const auto& data = leg_controller->datas[probe_leg];
      const auto& cmd = leg_controller->commands[probe_leg];
      std::array<double, 3> p_delta{};
      for (int i = 0; i < 3; ++i) {
        p_delta[i] = static_cast<double>(cmd.pDes[i] - data.p[i]);
      }
      std::array<double, 3> q_feedback{};
      std::array<double, 3> q_des_pub{};
      std::array<double, 3> tau_pub{};
      for (int joint = 0; joint < 3; ++joint) {
        const int dds = legbot::DdsJointIndexFromModel(probe_leg, joint);
        q_feedback[joint] = snapshot.dds_q[static_cast<std::size_t>(dds)];
        q_des_pub[joint] = guard.pub.q[static_cast<std::size_t>(dds)];
        tau_pub[joint] = guard.pub.tau[static_cast<std::size_t>(dds)];
      }
      std::cout << "[FOOT-DELTA-PROBE]"
                << " elapsed=" << std::fixed << std::setprecision(3)
                << elapsed
                << " leg=" << probe_leg << "(" << ModelLegName(probe_leg)
                << ") axis=" << args.foot_probe_axis
                << " requested_delta=" << args.foot_probe_delta
                << " p=" << legbot::ArrayText(Vec3TextArray(data.p), 5)
                << " pDes=" << legbot::ArrayText(Vec3TextArray(cmd.pDes), 5)
                << " pDes_minus_p=" << legbot::ArrayText(p_delta, 5)
                << " q_feedback=" << legbot::ArrayText(q_feedback, 5)
                << " q_des_pub=" << legbot::ArrayText(q_des_pub, 5)
                << " tau_pub=" << legbot::ArrayText(tau_pub, 5)
                << " num_qdes_clamped=" << guard.num_qdes_clamped
                << " num_tau_clamped=" << guard.num_tau_clamped
                << " tilt=" << state_adapter->Tilt(snapshot) << "\n";
      next_print = now + std::chrono::duration_cast<legbot::Clock::duration>(
  ...
  const auto hold_snapshot = state_adapter->Snapshot();
  const auto hold_command = command_adapter->MakeQOnlyCommand(
      hold_snapshot.dds_q, args.standup_kp, args.standup_kd);
  for (int i = 0; i < 20 && g_running; ++i) {
    if (!command_adapter->PublishLowCmd(lowcmd_pub, hold_command, stats,
                                        stats_t0)) {
      return "publish_failed:foot_delta_probe_hold";
    }
    std::this_thread::sleep_for(std::chrono::duration<double>(period_s));
  }
  std::cout << "[FOOT-DELTA-PROBE] completed publish=current_q_hold\n";
  return "foot_delta_probe_completed";
}
...
void PrintFootProbeLog(const FootProbeState& probe, double elapsed_s,
                       double alpha,
                       const legbot::LowStateSnapshot& snapshot,
                       const legbot::LegBotDDSStateAdapter& state_adapter,
                       const LegController<float>* leg_controller,
                       const legbot::OutputGuardStats& guard,
                       const FootProbeDiagnostics& diag,
                       bool compute_only = false) {
  if (!leg_controller) return;
  const auto& data = leg_controller->datas[probe.leg];
  const auto& cmd = leg_controller->commands[probe.leg];
  std::array<double, 3> q_feedback{};
  std::array<double, 3> q_des_raw{};
  std::array<double, 3> q_des_pub{};
  std::array<double, 3> q_des_raw_minus_feedback{};
  std::array<double, 3> tau_raw{};
  std::array<double, 3> tau_pub{};
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(probe.leg, joint);
    const auto idx = static_cast<std::size_t>(dds);
    q_feedback[joint] = snapshot.dds_q[idx];
    q_des_raw[joint] = guard.raw.q[idx];
    q_des_pub[joint] = guard.pub.q[idx];
    q_des_raw_minus_feedback[joint] = guard.raw.q[idx] - snapshot.dds_q[idx];
    tau_raw[joint] = guard.raw.tau[idx];
    tau_pub[joint] = guard.pub.tau[idx];
  }

  std::cout << "[FOOT-PROBE]"
            << " elapsed=" << std::fixed << std::setprecision(3)
            << elapsed_s
            << " compute_only=" << (compute_only ? "true" : "false")
            << " leg=" << probe.leg << "(" << ModelLegName(probe.leg) << ")"
            << " axis=" << FootProbeAxisName(probe.axis)
            << " target_delta=" << probe.delta
            << " alpha=" << alpha
            << " pFoot0=" << legbot::ArrayText(Vec3TextArray(probe.p_foot0), 5)
            << " pDes=" << legbot::ArrayText(Vec3TextArray(cmd.pDes), 5)
            << " pFoot_current=" << legbot::ArrayText(Vec3TextArray(data.p), 5)
            << " p_error=" << legbot::ArrayText(diag.p_error, 5)
            << " v_error=" << legbot::ArrayText(diag.v_error, 5)
            << " footForce=" << legbot::ArrayText(diag.foot_force, 5)
            << " J_rowmajor=" << legbot::ArrayText(diag.jacobian, 5)
            << " tau_cartesian="
            << legbot::ArrayText(diag.tau_cartesian_model, 5)
            << " tau_before_joint_scale="
            << legbot::ArrayText(diag.tau_before_joint_scale, 5)
            << " joint_tau_scale="
            << legbot::ArrayText(diag.joint_tau_scale, 3)
            << " tau_after_joint_scale="
            << legbot::ArrayText(diag.tau_after_joint_scale, 5)
            << " tau_after_joint_scale_calf05="
            << legbot::ArrayText(diag.tau_after_joint_scale_calf05, 5)
            << " tau_after_joint_scale_calf10="
            << legbot::ArrayText(diag.tau_after_joint_scale_calf10, 5)
            << " qDes_source=" << diag.qdes_source
            << " kpCartesian="
            << legbot::ArrayText(Mat3DiagTextArray(cmd.kpCartesian), 3)
            << " kdCartesian="
            << legbot::ArrayText(Mat3DiagTextArray(cmd.kdCartesian), 3)
            << " kpJoint=" << legbot::ArrayText(Mat3DiagTextArray(cmd.kpJoint), 3)
            << " kdJoint=" << legbot::ArrayText(Mat3DiagTextArray(cmd.kdJoint), 3)
            << " q_feedback=" << legbot::ArrayText(q_feedback, 5)
            << " q_des_raw=" << legbot::ArrayText(q_des_raw, 5)
            << " q_des_pub=" << legbot::ArrayText(q_des_pub, 5)
            << " q_des_raw_minus_feedback="
            << legbot::ArrayText(q_des_raw_minus_feedback, 5)
            << " tau_raw=" << legbot::ArrayText(tau_raw, 5)
            << " tau_pub=" << legbot::ArrayText(tau_pub, 5)
            << " num_qdes_clamped=" << guard.num_qdes_clamped
            << " num_tau_clamped=" << guard.num_tau_clamped
...
void FillFootProbeLegCommands(const FootProbeState& probe, double alpha,
                              double stand_kp, double stand_kd,
                              double cart_kp, double cart_kd,
                              LegController<float>* leg_controller) {
  if (!leg_controller) return;
  const auto model_q_stand = legbot::DdsQToModelQ(legbot::StandQDds());
  for (int leg = 0; leg < 4; ++leg) {
    auto& cmd = leg_controller->commands[leg];
    const auto& data = leg_controller->datas[leg];
    cmd.zero();
    cmd.qDes << static_cast<float>(model_q_stand[legbot::ModelJointIndex(
                    leg, static_cast<int>(legbot::ModelJoint::Hip))]),
        static_cast<float>(model_q_stand[legbot::ModelJointIndex(
            leg, static_cast<int>(legbot::ModelJoint::Thigh))]),
        static_cast<float>(model_q_stand[legbot::ModelJointIndex(
            leg, static_cast<int>(legbot::ModelJoint::Calf))]);
    cmd.qdDes.setZero();
    cmd.kpJoint = Mat3<float>::Identity() * static_cast<float>(stand_kp);
    cmd.kdJoint = Mat3<float>::Identity() * static_cast<float>(stand_kd);
    cmd.pDes = data.p;
    cmd.vDes.setZero();
    if (leg == probe.leg) {
      cmd.qDes = data.q;
      cmd.kpJoint.setZero();
      cmd.kdJoint.setZero();
      cmd.pDes = probe.p_foot0;
      cmd.pDes[probe.axis] += static_cast<float>(probe.delta * alpha);
      cmd.kpCartesian =
          Mat3<float>::Identity() * static_cast<float>(cart_kp);
      cmd.kdCartesian =
          Mat3<float>::Identity() * static_cast<float>(cart_kd);
    }
  }
}

std::array<double, 3> SelectedLegDdsArray(
    const std::array<double, legbot::kNumJoints>& values, int leg) {
  std::array<double, 3> out{};
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(leg, joint);
    out[static_cast<std::size_t>(joint)] =
        values[static_cast<std::size_t>(dds)];
  }
  return out;
}

std::array<double, 3> SelectedLegTauBeforeScale(const SpiCommand& spi_command,
                                                int leg) {
  return {static_cast<double>(spi_command.tau_abad_ff[leg]),
          static_cast<double>(spi_command.tau_hip_ff[leg]),
          static_cast<double>(spi_command.tau_knee_ff[leg])};
}

std::array<double, 3> SelectedLegTauScale(int leg) {
  std::array<double, 3> out{};
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(leg, joint);
    out[static_cast<std::size_t>(joint)] =
...
void CopySelectedLegCommand(const legbot::DdsCommandArrays& src, int leg,
                            legbot::DdsCommandArrays* dst) {
  if (!dst) return;
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(leg, joint);
    const auto idx = static_cast<std::size_t>(dds);
    dst->q[idx] = src.q[idx];
    dst->dq[idx] = src.dq[idx];
    dst->kp[idx] = src.kp[idx];
    dst->kd[idx] = src.kd[idx];
    dst->tau[idx] = src.tau[idx];
    dst->mode[idx] = src.mode[idx];
  }
}

int CountSelectedLegChanged(const std::array<double, legbot::kNumJoints>& a,
                            const std::array<double, legbot::kNumJoints>& b,
                            int leg) {
  int count = 0;
  for (int joint = 0; joint < 3; ++joint) {
    const int dds = legbot::DdsJointIndexFromModel(leg, joint);
    if (a[static_cast<std::size_t>(dds)] !=
        b[static_cast<std::size_t>(dds)]) {
      ++count;
    }
  }
  return count;
...
void PrintSingleLegAlgoLog(
    const SingleLegAlgoProbeState& probe, double elapsed_s,
    double requested_gait, const MIT_UserParameters* user_params,
    Key3Phase key3_phase, const legbot::LowStateSnapshot& snapshot,
    const legbot::LegBotDDSStateAdapter& state_adapter,
    const legbot::DdsCommandArrays& raw_algo,
    const legbot::DdsCommandArrays& masked_raw,
    const legbot::OutputGuardStats& guard,
    const SpiCommand& spi_command, const MIT_Controller* controller) {
  const auto* cmpc = controller ? controller->debugConvexMPC() : nullptr;
  const bool cmpc_valid = cmpc && cmpc->valid;
  const std::array<double, 4> swing_phase =
      cmpc_valid ? Vec4TextArray(cmpc->swingStates)
                 : std::array<double, 4>{};
  const std::array<double, 4> mpc_table_now =
      cmpc_valid ? Vec4iTextArray(cmpc->mpcTableNow)
                 : std::array<double, 4>{};
  const auto q_feedback = SelectedLegDdsArray(snapshot.dds_q, probe.leg);
  const auto q_des_raw_algo = SelectedLegDdsArray(raw_algo.q, probe.leg);
  const auto qd_des_raw_algo = SelectedLegDdsArray(raw_algo.dq, probe.leg);
  const auto kp_raw_algo = SelectedLegDdsArray(raw_algo.kp, probe.leg);
  const auto kd_raw_algo = SelectedLegDdsArray(raw_algo.kd, probe.leg);
  const auto tau_raw_algo_before_scale =
      SelectedLegTauBeforeScale(spi_command, probe.leg);
  const auto joint_tau_scale = SelectedLegTauScale(probe.leg);
  const auto tau_after_joint_scale =
      SelectedLegDdsArray(raw_algo.tau, probe.leg);
  const auto q_des_pub = SelectedLegDdsArray(guard.pub.q, probe.leg);
  const auto tau_pub = SelectedLegDdsArray(guard.pub.tau, probe.leg);
  std::array<double, 3> q_des_raw_minus_feedback{};
  for (int i = 0; i < 3; ++i) {
    q_des_raw_minus_feedback[static_cast<std::size_t>(i)] =
        q_des_raw_algo[static_cast<std::size_t>(i)] -
        q_feedback[static_cast<std::size_t>(i)];
  }
  const int selected_qdes_clamped =
      CountSelectedLegChanged(masked_raw.q, guard.pub.q, probe.leg);
  const int selected_tau_clamped =
      CountSelectedLegChanged(masked_raw.tau, guard.pub.tau, probe.leg);

  std::cout << "[SINGLE-LEG-ALGO]"
            << " elapsed=" << std::fixed << std::setprecision(3)
            << elapsed_s
            << " probe_leg=" << probe.leg
            << " probe_leg_name=" << ModelLegName(probe.leg)
            << " requested_gait=" << requested_gait
            << " actual_cmpc_gait="
            << (user_params ? user_params->cmpc_gait : -1.0)
            << " phase=SINGLE_LEG_ALGO_PROBE"
            << " key3_phase=" << Key3PhaseText(key3_phase)
            << " swing_phase="
            << (cmpc_valid ? legbot::ArrayText(swing_phase, 3)
                           : "unavailable")
            << " contact_table="
            << (cmpc_valid ? legbot::ArrayText(mpc_table_now, 0)
                           : "unavailable")
            << " mpc_table_now="
            << (cmpc_valid ? legbot::ArrayText(mpc_table_now, 0)
                           : "unavailable")
            << " source=algorithm_raw_for_selected_leg_qstand_for_others"
            << " q_feedback=" << legbot::ArrayText(q_feedback, 5)
            << " q_des_raw_algo=" << legbot::ArrayText(q_des_raw_algo, 5)
            << " qd_des_raw_algo=" << legbot::ArrayText(qd_des_raw_algo, 5)
            << " kp_raw_algo=" << legbot::ArrayText(kp_raw_algo, 3)
            << " kd_raw_algo=" << legbot::ArrayText(kd_raw_algo, 3)
            << " tau_raw_algo_before_scale="
            << legbot::ArrayText(tau_raw_algo_before_scale, 5)
            << " joint_tau_scale=" << legbot::ArrayText(joint_tau_scale, 3)
            << " tau_after_joint_scale="
            << legbot::ArrayText(tau_after_joint_scale, 5)
            << " q_des_pub=" << legbot::ArrayText(q_des_pub, 5)
            << " tau_pub=" << legbot::ArrayText(tau_pub, 5)
            << " q_des_raw_minus_feedback="
            << legbot::ArrayText(q_des_raw_minus_feedback, 5)
            << " num_qdes_clamped=" << selected_qdes_clamped
            << " num_tau_clamped=" << selected_tau_clamped
            << " other_leg_sources=q_stand_hold"
            << " tilt=" << state_adapter.Tilt(snapshot)
            << " hard_abort_reason=none"
            << "\n";
}
...
std::string RunFootDeltaProbeComputeOnly(
    const legbot::BridgeArgs& args, RobotRunner* runner, SpiData* spi_data,
    SpiCommand* spi_command, VectorNavData* vector_nav_data,
    CheaterState<double>* cheater_state,
    legbot::LegBotDDSStateAdapter* state_adapter,
    ...
    const legbot::Clock::time_point& stats_t0) {
  if (!runner || !spi_data || !spi_command || !vector_nav_data ||
      !cheater_state || !state_adapter || !command_adapter || !stats) {
    return "foot_delta_probe_compute_only_missing_dependency";
  }
  auto* leg_controller = runner->debugMutableLegController();
  if (!leg_controller) return "foot_delta_probe_missing_leg_controller";

  FootProbeState probe;
  probe.leg = args.foot_probe_leg;
  probe.axis = 2;
  if (args.foot_probe_axis == "x") probe.axis = 0;
  if (args.foot_probe_axis == "y") probe.axis = 1;
  probe.delta = args.foot_probe_delta;
  probe.duration = args.foot_probe_seconds;

  std::array<double, legbot::kNumJoints> previous_q_des = legbot::StandQDds();
  std::array<double, legbot::kNumJoints> previous_qd_des{};
  bool has_previous_q_des = true;
  auto next = legbot::Clock::now();
  auto last_sample_t = next;
  auto last_tick_change_t = next;
  uint32_t last_lowstate_tick = std::numeric_limits<uint32_t>::max();
  ...
  std::cout << "[FOOT-PROBE-COMPUTE-ONLY] start"
            << " leg=" << probe.leg << "(" << ModelLegName(probe.leg) << ")"
            << " axis=" << FootProbeAxisName(probe.axis)
            << " delta=" << probe.delta
  ...
  while (g_running) {
    const auto now = legbot::Clock::now();
    const double elapsed = legbot::SecondsSince(start, now);
    if (elapsed >= probe.duration) break;
    if (state_adapter->IsTimeout()) {
      return "foot_delta_probe_compute_only_failed:rt_lowstate_timeout";
    }
    ...
    const auto tick_safety = CheckTickAdvances(
        snapshot, args, now, &last_lowstate_tick, &last_tick_change_t);
    if (!tick_safety.ok) {
      return std::string("foot_delta_probe_compute_only_failed:") +
             tick_safety.reason;
    }
    legbot::RecordLowStateTiming(snapshot.tick, stats, &last_sample_t,
                               &last_recorded_tick);
    ...
    if (!probe.active) {
      probe.active = true;
      probe.start_time = now;
      probe.next_print = now;
      probe.p_foot0 = leg_controller->datas[probe.leg].p;
      for (int joint = 0; joint < 3; ++joint) {
        const int dds = legbot::DdsJointIndexFromModel(probe.leg, joint);
        probe.q_feedback_at_start[static_cast<std::size_t>(joint)] =
            snapshot.dds_q[static_cast<std::size_t>(dds)];
      }
    }

    const double probe_elapsed = legbot::SecondsSince(probe.start_time, now);
    const double alpha = FootProbeAlpha(probe_elapsed, probe.duration);
    FillFootProbeLegCommands(probe, alpha, args.standup_kp, args.standup_kd,
                             args.foot_probe_kp, args.foot_probe_kd,
                             leg_controller);
    leg_controller->updateCommand(spi_command);
    auto raw_command = command_adapter->FromSpiCommand(*spi_command);
    command_adapter->ApplyTauScale(args.tau_ff_scale, &raw_command);
    const auto diag = CaptureFootProbeDiagnostics(
        probe.leg, *leg_controller, *spi_command, raw_command,
        args.tau_ff_scale,
        "feedback_current_q_zero_joint_kp_kd_cartesian_tau_only");
    legbot::OutputGuardStats guard;
    if (!legbot::ApplyOutputGuard(
            raw_command, args, has_previous_q_des ? &previous_q_des : nullptr,
            has_previous_q_des ? &previous_qd_des : nullptr, &guard)) {
      std::cout << "[FOOT-PROBE-COMPUTE-ONLY] output_guard_fault="
                << guard.fault_reason << "\n";
    }
    previous_q_des = guard.pub.q;
    previous_qd_des = guard.pub.dq;
    ...
    if (now >= probe.next_print) {
      PrintFootProbeLog(probe, probe_elapsed, alpha, snapshot, *state_adapter,
                        leg_controller, guard, diag, true);
      probe.next_print =
          now + std::chrono::duration_cast<legbot::Clock::duration>(
                    std::chrono::duration<double>(0.2));
    }
  ...
  std::cout << "[FOOT-PROBE-COMPUTE-ONLY] done publish=none\n";
  (void)stats_t0;
  return "foot_delta_probe_compute_only_completed";
}
...
std::string RunInteractiveControl(const legbot::BridgeArgs& args,
  ...
  bool single_leg_release_gate_scope_logged = false;
  ...
  FootProbeState foot_probe;
  SingleLegAlgoProbeState single_leg_probe;
  ...
  auto set_key3_phase = [&](Key3Phase next_phase,
    ...
    if (next_phase == Key3Phase::kGait0Shadow) {
      ...
      single_leg_release_gate_scope_logged = false;
  ...
  std::cout
      ...
      << "  keys: 1 q-only stand-up, 2 MIT balance-stand direct, "
         "3 MIT forward direct, p selected-leg algorithm output probe, "
         "x q-stand hold, 4 down+disable+exit, "
         "q down+disable+exit\n";
  ...
  while (g_running) {
    ...
    if (key >= 0) {
      switch (key) {
        case '1':
          std::cout << "\n[LEGBOT-DDS][KEY] 1: q-only stand-up ramp from current feedback\n";
          RunGuardedStandup(args, state_adapter, lowcmd_pub, command_adapter,
                            stats, stats_t0);
          standing_ready = true;
          mode = Mode::kQHold;
          foot_probe = FootProbeState{};
          single_leg_probe = SingleLegAlgoProbeState{};
          key3_phase = Key3Phase::kNone;
          gait0_has_previous_raw_q = false;
          gait0_raw_stable_count = 0;
          gait0_raw_released = false;
        ...
        case 'P':
          if (!standing_ready) {
            std::cout << "\n[SINGLE-LEG-ALGO] rejected: standup not complete\n";
            break;
          }
          if (mode != Mode::kQHold) {
            std::cout << "\n[SINGLE-LEG-ALGO] rejected: mode not q_stand_hold\n";
            break;
          }
          {
            const auto fsm_state = CurrentFsmStateName(controller);
            const bool already_balance_stand =
                fsm_state == FSM_StateName::BALANCE_STAND;
            std::cout << "\n[SINGLE-LEG-ALGO][KEY] p: reuse key3 init flow"
                      << " (arming -> gait4 shadow -> gait4 direct ->"
                      << " resetConvexMPC -> gait0 shadow compute-only),"
                      << " then single-leg raw output"
                      << " probe_leg=" << args.single_leg_probe_leg
                      << "(" << ModelLegName(args.single_leg_probe_leg) << ")"
                      << " duration=" << args.single_leg_probe_seconds
                      << " requested_gait=" << args.test_gait
                      << " fsm_current=" << FsmStateNameText(fsm_state)
                      << "\n";
            single_leg_probe = SingleLegAlgoProbeState{};
            single_leg_probe.active = true;
            single_leg_probe.leg = args.single_leg_probe_leg;
            single_leg_probe.duration = args.single_leg_probe_seconds;
            mode = Mode::kKey3Locomotion;
            foot_probe = FootProbeState{};
            gait0_has_previous_raw_q = false;
            gait0_raw_stable_count = 0;
    ...
    if (mode == Mode::kQHold) {
    ...
    } else if (mode == Mode::kSingleLegAlgoProbe) {
      // p key: single-leg raw algorithm output probe.
      // Reached after key3 init flow (arming -> gait4 shadow -> gait4 direct
      // -> resetConvexMPC -> gait0 shadow compute-only) confirmed the raw
      // command is valid. The algorithm keeps running all four legs; only
      // the final publish mixes selected-leg raw with Q-STAND-HOLD for the
      // other three legs. No ApplyOutputGuard, no qDes/tau/kp/kd clamp, no
      // blend. Hard safety retained: NaN/Inf check, physical hard range
      // check (reject, not clamp), feedback-side hard safety (checked above).
      state_adapter->FillCheetahData(snapshot, spi_data, vector_nav_data);
      FillCheaterFromLowState(snapshot, cheater_state);
      ApplyControllerCommand(ControllerPhase::kLocomotionTest, args.test_gait,
                             0., 0., 0., robot_params, user_params,
                             gamepad_command);
      runner->run();

      if (!single_leg_probe.raw_publish_started) {
        single_leg_probe.raw_publish_started = true;
        single_leg_probe.start_time = now;
        single_leg_probe.next_print = now;
        std::cout << "[SINGLE-LEG-ALGO] raw publish start leg="
                  << single_leg_probe.leg << "("
                  << ModelLegName(single_leg_probe.leg) << ")"
                  << " duration=" << single_leg_probe.duration
                  << " requested_gait=" << args.test_gait << "\n";
      }

      const double probe_elapsed =
          legbot::SecondsSince(single_leg_probe.start_time, now);
      if (probe_elapsed >= single_leg_probe.duration) {
        mode = Mode::kQHold;
        single_leg_probe = SingleLegAlgoProbeState{};
        previous_q_des = legbot::StandQDds();
        previous_qd_des.fill(0.);
        has_previous_q_des = true;
        next_print = now;
        std::cout << "[SINGLE-LEG-ALGO] done -> Q_STAND_HOLD\n";
        continue;
      }

      auto raw_algo = command_adapter->FromSpiCommand(*spi_command);
      command_adapter->ApplyTauScale(args.tau_ff_scale, &raw_algo);

      // Readiness check: don't publish uninitialized/zero raw command.
      // If raw command is all-zero, kp/kd all-zero, cmpc_debug unavailable,
      // or FSM not in Locomotion, hold Q-STAND-HOLD and don't publish raw.
      const auto readiness = InspectMitOutputReadiness(raw_algo);
      const auto* cmpc_debug =
          controller ? controller->debugConvexMPC() : nullptr;
      const bool cmpc_valid = cmpc_debug && cmpc_debug->valid;
      const auto fsm_state = CurrentFsmStateName(controller);
      const bool fsm_in_locomotion = fsm_state == FSM_StateName::LOCOMOTION;

      if (!readiness.ready || !cmpc_valid || !fsm_in_locomotion) {
        const auto hold_command = command_adapter->MakeQOnlyCommand(
            key3_hold_q, args.standup_kp, args.standup_kd);
        if (!command_adapter->PublishLowCmd(lowcmd_pub, hold_command, stats,
                                            stats_t0)) {
          std::cerr << "[LEGBOT-DDS][ERROR] publish_failed during single-leg-algo hold\n";
          command_adapter->DisableBurst(lowcmd_pub, args, stats, stats_t0);
          return "publish_failed:single-leg-algo-hold";
        }
        previous_q_des = hold_command.q;
        previous_qd_des = hold_command.dq;
        has_previous_q_des = true;
        if (now >= single_leg_probe.next_print) {
          const std::string hold_reason =
              !readiness.ready ? readiness.reason
              : !cmpc_valid ? "cmpc_debug_unavailable"
              : "fsm_not_in_locomotion";
          std::cout << "[SINGLE-LEG-ALGO] hold reason=" << hold_reason
                    << " elapsed=" << std::fixed << std::setprecision(3)
                    << probe_elapsed
                    << " publish=Q-STAND-HOLD\n";
          single_leg_probe.next_print =
              now + std::chrono::duration_cast<legbot::Clock::duration>(
                        std::chrono::duration<double>(0.2));
        }
      } else {
        // Raw command valid: single-leg mixing + raw publish.
        auto masked_command = command_adapter->MakeQOnlyCommand(
            key3_hold_q, args.standup_kp, args.standup_kd);
        CopySelectedLegCommand(raw_algo, single_leg_probe.leg,
                               &masked_command);

        // Physical hard range check (reject, not clamp). If the selected
        // leg's raw qDes or tau exceeds the physical hard range, print the
        // reason and go back to Q-STAND-HOLD. Do NOT clamp and continue
        // publishing -- that would distort the original algorithm output.
        std::string range_fault;
        for (int joint = 0; joint < 3 && range_fault.empty(); ++joint) {
          const int dds =
              legbot::DdsJointIndexFromModel(single_leg_probe.leg, joint);
          const int model_joint = legbot::DdsToModelJoint(dds);
          const auto q_limit =
              legbot::CmdQDesLimitForModelJoint(model_joint, args);
          const double q = masked_command.q[static_cast<std::size_t>(dds)];
          if (!std::isfinite(q) || q < q_limit.first || q > q_limit.second) {
            range_fault = std::string("selected_leg_qdes_out_of_range:") +
                          legbot::JointNameDds(dds) +
                          " q=" + std::to_string(q) +
                          " limit=[" + std::to_string(q_limit.first) +
                          "," + std::to_string(q_limit.second) + "]";
            break;
          }
          if (args.cmd_tau_limit > 0.) {
            const double tau =
                masked_command.tau[static_cast<std::size_t>(dds)];
            if (!std::isfinite(tau) ||
                std::abs(tau) > args.cmd_tau_limit) {
              range_fault =
                  std::string("selected_leg_tau_out_of_range:") +
                  legbot::JointNameDds(dds) +
                  " tau=" + std::to_string(tau) +
                  " limit=" + std::to_string(args.cmd_tau_limit);
              break;
            }
          }
        }
        if (!range_fault.empty()) {
          std::cerr << "[SINGLE-LEG-ALGO][HARD-RANGE-FAULT] " << range_fault
                    << "; back to Q-STAND-HOLD (not clamping)\n";
          mode = Mode::kQHold;
          single_leg_probe = SingleLegAlgoProbeState{};
          previous_q_des = legbot::StandQDds();
          previous_qd_des.fill(0.);
        ...
        // NaN/Inf hard safety check (no clamps). PopulateRawCommandStats
        // copies raw -> pub without any clamping.
        legbot::OutputGuardStats guard;
        if (!PopulateRawCommandStats(masked_command, &guard)) {
        ...
        if (!command_adapter->PublishLowCmd(lowcmd_pub, guard.pub, stats,
                                            stats_t0)) {
          std::cerr << "[LEGBOT-DDS][ERROR] publish_failed during single-leg-algo raw\n";
          command_adapter->DisableBurst(lowcmd_pub, args, stats, stats_t0);
          return "publish_failed:single-leg-algo-raw";
        }
        previous_q_des = guard.pub.q;
        previous_qd_des = guard.pub.dq;
        has_previous_q_des = true;
        ...
        if (now >= single_leg_probe.next_print) {
          PrintSingleLegAlgoLog(
              single_leg_probe, probe_elapsed, args.test_gait, user_params,
              key3_phase, snapshot, *state_adapter, raw_algo, masked_command,
              guard, *spi_command, controller);
          single_leg_probe.next_print =
              now + std::chrono::duration_cast<legbot::Clock::duration>(
                        std::chrono::duration<double>(0.2));
        }
      ...
      const bool single_leg_release_scope =
          single_leg_probe.active &&
          key3_phase == Key3Phase::kGait0Shadow &&
          single_leg_probe.leg >= 0 && single_leg_probe.leg < 4;
      const int release_gate_leg =
          single_leg_release_scope ? single_leg_probe.leg : -1;
      if (single_leg_release_scope &&
          !single_leg_release_gate_scope_logged) {
        single_leg_release_gate_scope_logged = true;
        std::cout << "[SINGLE-LEG-ALGO] release gate checks selected leg only:"
                  << " leg=" << release_gate_leg << "("
                  << ModelLegName(release_gate_leg) << ")\n";
      }
      const auto gait0_entry_violation =
          single_leg_release_scope
              ? MaxAbsQStepForLeg(raw_command, gait0_entry_q_fb,
                                  release_gate_leg)
              : MaxAbsQStep(raw_command, gait0_entry_q_fb);
      const auto gait0_raw_vs_hold_step =
          single_leg_release_scope
              ? MaxAbsQStepForLeg(raw_command, gait0_entry_q_hold,
                                  release_gate_leg)
              : MaxAbsQStep(raw_command, gait0_entry_q_hold);
      const double gait0_entry_warn_limit =
          gait0_entry_violation.joint >= 0
              ? ReadyMaxQErrorForDdsJoint(
                    gait0_entry_violation.joint,
                    true /* gait0_shadow_entry_check */)
              : std::numeric_limits<double>::infinity();
      const bool gait0_raw_jump_warn_only =
          gait0_entry_violation.joint >= 0 &&
          std::isfinite(gait0_entry_violation.value) &&
          gait0_entry_violation.value > gait0_entry_warn_limit &&
          gait0_entry_violation.value <=
              Gait0ShadowHardQJumpLimitForDdsJoint(
                  gait0_entry_violation.joint);
      const double gait0_raw_vs_hold_hard_limit =
          gait0_raw_vs_hold_step.joint >= 0
              ? Gait0ShadowHardQJumpLimitForDdsJoint(
                    gait0_raw_vs_hold_step.joint)
              : std::numeric_limits<double>::infinity();
      const bool gait0_raw_vs_hold_hard_block =
          gait0_raw_vs_hold_step.joint >= 0 &&
          (!std::isfinite(gait0_raw_vs_hold_step.value) ||
           gait0_raw_vs_hold_step.value > gait0_raw_vs_hold_hard_limit);
      ...
      if (gait0_has_previous_raw_q) {
        gait0_raw_q_step =
            single_leg_release_scope
                ? MaxAbsQStepForLeg(raw_command, gait0_previous_raw_q,
                                    release_gate_leg)
                : MaxAbsQStep(raw_command, gait0_previous_raw_q);
      }
      const double gait0_max_abs_raw_qd =
          single_leg_release_scope
              ? MaxAbsRawQdForLeg(raw_command, release_gate_leg)
              : MaxAbsRawQd(raw_command);
      const double gait0_max_abs_raw_tau =
          single_leg_release_scope
              ? MaxAbsRawTauForLeg(raw_command, release_gate_leg)
              : MaxAbsRawTau(raw_command);
      const bool gait0_raw_tau_warn_only =
          std::isfinite(gait0_max_abs_raw_tau) &&
          gait0_max_abs_raw_tau >= kGait0ShadowMaxAbsTau &&
          gait0_max_abs_raw_tau < kGait0ShadowHardAbsTau;
      ...
      const double selected_cmpc_foot_jump_diagnostic =
          single_leg_release_scope
              ? DebugMaxCmpcFootPDesJumpForLeg(cmpc_debug_for_diag,
                                               release_gate_leg)
              : std::numeric_limits<double>::quiet_NaN();
      const double selected_leg_command_foot_jump_diagnostic =
          single_leg_release_scope
              ? DebugMaxFootPDesJumpForLeg(runner, release_gate_leg)
              : std::numeric_limits<double>::quiet_NaN();
      const double foot_jump_diagnostic =
          single_leg_release_scope
              ? ((gait0_phase &&
                  std::isfinite(selected_cmpc_foot_jump_diagnostic))
                     ? selected_cmpc_foot_jump_diagnostic
                     : selected_leg_command_foot_jump_diagnostic)
      ...
      if (gait0_compute_only) {
        bridge_ready = InspectGait0ShadowReleaseGate(
            ...
            foot_jump_diagnostic, release_gate_leg);
        ...
        if (bridge_ready.ready && gait0_raw_vs_hold_hard_block) {
          std::ostringstream oss;
          oss << "raw_vs_hold_extreme_jump joint="
              << (gait0_raw_vs_hold_step.joint >= 0
                      ? legbot::JointNameDds(gait0_raw_vs_hold_step.joint)
                      : "unknown")
              << " diff=" << gait0_raw_vs_hold_step.value
              << " hard_limit=" << gait0_raw_vs_hold_hard_limit
              << " entry_reference_source=Q_STAND_HOLD";
          bridge_ready.ready = false;
          bridge_ready.reject_code = BridgeRejectCode::kQDesJump;
          bridge_ready.reason = oss.str();
        }
        ...
        if (bridge_ready.ready &&
          ...
          const bool entry_increase_hard_block = !gait0_raw_step_stable;
          std::cout << "[GAIT0-RELEASE-GATE-DIAG]"
                    << " reason="
                    ...
                    << " raw_jump_warn_only="
                    << (gait0_raw_jump_warn_only ? "true" : "false")
                    << " stable_count="
        ...
        if (bridge_ready.ready) {
          ...
          if (!cmpc || !cmpc->valid) {
            ...
            const int first_boundary_leg =
                single_leg_release_scope ? release_gate_leg : 0;
            const int end_boundary_leg =
                single_leg_release_scope ? release_gate_leg + 1 : 4;
            for (int leg = first_boundary_leg; leg < end_boundary_leg; ++leg) {
        ...
        if (bridge_ready.ready &&
          ...
          if (!single_leg_probe.active &&
              (gait0_raw_jump_warn_only || gait0_raw_tau_warn_only)) {
            std::cout << "[GAIT0-RELEASE] allow blend_from_hold"
                      << " release_mode=blend_from_hold"
                      << " entry_reference_q_hold="
                      << legbot::ArrayText(gait0_entry_q_hold, 5)
                      << " last_safe_q_des="
                      << legbot::ArrayText(key3_last_safe_command.q, 5)
                      << " q_des_jump_warn_only="
                      << (gait0_raw_jump_warn_only ? "true" : "false")
                      << " raw_tau_warn_only="
                      << (gait0_raw_tau_warn_only ? "true" : "false")
                      << " max_entry_violation="
                      << gait0_entry_violation.value
                      << " max_entry_violation_joint="
                      << (gait0_entry_violation.joint >= 0
                              ? legbot::JointNameDds(gait0_entry_violation.joint)
                              : "unknown")
                      << " old_entry_warn_limit="
                      << gait0_entry_warn_limit
                      << " hard_q_jump_limit="
                      << (gait0_entry_violation.joint >= 0
                              ? Gait0ShadowHardQJumpLimitForDdsJoint(
                                    gait0_entry_violation.joint)
                      ...
                      << (gait0_raw_vs_hold_step.joint >= 0
                              ? legbot::JointNameDds(
                                    gait0_raw_vs_hold_step.joint)
                              : "unknown")
                      << " raw_vs_hold_hard_limit="
                      << gait0_raw_vs_hold_hard_limit
                      << " max_abs_raw_tau="
                      << gait0_max_abs_raw_tau
                      << " raw_tau_warn_limit="
                      << kGait0ShadowWarnAbsTau
                      << " raw_tau_hard_limit="
                      << kGait0ShadowHardAbsTau
                      << " raw_q_step="
                      << gait0_raw_q_step.value
                      << " raw_q_step_limit="
                      << kGait0RawStableQStepLimit
                      << " mpc_table_transitions="
                      << gait0_window_mpc_table_transitions
                      << "\n";
          }
        } else {
      ...
      const bool gait0_blend_entry_soft_hold = false;
      const char* gait0_blend_entry_envelope_mode =
          !gait0_blend_phase
              ? "hard_block"
              : (gait0_blend_entry_violation ? "warn_only_minimal"
                                             : "warn_only");
      ...
      auto apply_gait0_blend_qdes_takeover = [&]() {
        if (key3_phase != Key3Phase::kGait0Blend) return;

        gait0_blend_alpha_q = 1.0;
        alpha_q = 1.0;
        alpha_qd = 1.0;
        alpha_tau = 1.0;
        ...
        for (std::size_t i = 0; i < legbot::kNumJoints; ++i) {
          ...
          command_for_guard.q[i] =
              q_prev + legbot::Clamp(raw_command.q[i] - q_prev,
                                   -joint_q_step_limit,
                                   joint_q_step_limit);

          command_for_guard.dq[i] = raw_command.dq[i];
          command_for_guard.tau[i] = raw_command.tau[i];
          if (stance_leg) {
        ...
        publish_tag = "GAIT0_BLEND_QDES_TAKEOVER";
      };
      ...
      if (shadow_phase) {
        ...
        if (key3_phase == Key3Phase::kGait0Shadow &&
            gait0_raw_released) {
          if (single_leg_probe.active) {
            // p key: raw command confirmed valid by gait0_shadow
            // compute-only. Branch to single-leg raw publish instead of
            // gait0_blend. Algorithm keeps running all four legs; only
            // the final publish mixes selected-leg raw with Q-STAND-HOLD
            // for the other three. No ApplyOutputGuard, no clamp, no
            // blend.
            mode = Mode::kSingleLegAlgoProbe;
            single_leg_probe.raw_publish_started = false;
            single_leg_probe.start_time = now;
            single_leg_probe.next_print = now;
            previous_q_des = key3_last_safe_command.q;
            previous_qd_des = key3_last_safe_command.dq;
            has_previous_q_des = true;
            std::cout << "[SINGLE-LEG-ALGO] gait0_shadow release ready;"
                      << " switch to single-leg raw publish"
                      << " leg=" << single_leg_probe.leg
                      << "(" << ModelLegName(single_leg_probe.leg) << ")"
                      << " duration=" << single_leg_probe.duration
                      << " requested_gait=" << args.test_gait << "\n";
          } else {
            if (controller) controller->resetConvexMPCGaitPhase();
            const auto release_snapshot = state_adapter->Snapshot();
            key3_hold_q = legbot::StandQDds();
            key3_last_safe_command = command_adapter->MakeQOnlyCommand(
                key3_hold_q, args.standup_kp, args.standup_kd);
            gait0_entry_q_fb = release_snapshot.dds_q;
            gait0_entry_q_hold = key3_last_safe_command.q;
            gait0_previous_entry_violation =
                std::numeric_limits<double>::quiet_NaN();
            gait0_entry_violation_increase_count = 0;
            ...
            previous_qd_des = key3_last_safe_command.dq;
            has_previous_q_des = true;
            std::cout << "[GAIT0-RELEASE] reset ConvexMPC gait phase before GAIT0_BLEND"
                      << " release_mode=blend_from_hold"
                      ...
                      << " last_safe_q_des="
                      << legbot::ArrayText(key3_last_safe_command.q, 4)
                      << "\n";
        ...
        if (!bridge_ready.ready) {
          if (gait4_direct_raw_warmup) {
            publish_tag =
                single_leg_probe.active
                    ? "KEY3_LAST_SAFE_COMMAND_SINGLE_LEG_ARMING"
                    : "KEY3_LAST_SAFE_COMMAND_GAIT4_DIRECT_COMPUTE_ONLY";
            ...
            if (single_leg_probe.active &&
                key3_phase == Key3Phase::kLocomotionGait4Direct) {
              publish_tag = "Q-STAND-HOLD_SINGLE_LEG_ARMING";
            } else {
              publish_tag = key3_phase == Key3Phase::kLocomotionGait4Direct
                                ? "Q-STAND-HOLD_GAIT4_DIRECT_COMPUTE_ONLY"
                                : "KEY3_LAST_SAFE_COMMAND";
            }
          ...
          if (key3_phase == Key3Phase::kLocomotionGait4Direct) {
            const bool single_leg_arming = single_leg_probe.active;
            publish_tag =
                single_leg_arming
                    ? "Q-STAND-HOLD_SINGLE_LEG_ARMING"
                    : "KEY3_LAST_SAFE_COMMAND_GAIT4_DIRECT_COMPUTE_ONLY";
            const auto& gait4_direct_hold =
                single_leg_arming ? q_hold_command : key3_last_safe_command;
            publish_ok = publish_hold_with_raw_stats(gait4_direct_hold);
            alpha_q = 0.0;
            alpha_qd = 0.0;
            alpha_tau = 0.0;
            previous_q_des = guard.pub.q;
            ...
            if (phase_elapsed >= kKey3Gait4DirectSeconds &&
              ...
              if (gait4_direct_stable) {
                ...
                gait0_entry_q_hold = key3_last_safe_command.q;
                gait0_raw_released = false;
                std::cout << "[GAIT0-ENTRY] reset gait0 from current hold"
                          << " entry_reference_source=Q_STAND_HOLD"
                          << " gait4_raw_used_for_publish=false"
                          << " gait4_raw_used_for_entry_reference=false"
                          << " publish=" << publish_tag
                          << " entry_feedback_q="
                          << legbot::ArrayText(gait0_entry_q_fb, 4)
                          << " entry_reference_q_hold="
                          << legbot::ArrayText(gait0_entry_q_hold, 4)
                          << "\n";
                std::cout << (single_leg_arming
                                  ? "[SINGLE-LEG-ALGO]"
                                  : "[LEGBOT-DDS][KEY3-PHASE]")
                          << " reset ConvexMPC gait phase before GAIT0_SHADOW"
                          << " publish=" << publish_tag
                          << " firstSwing=reset swingTimeRemaining=0"
                          ...
                          << "\n";
                set_key3_phase(
                    Key3Phase::kGait0Shadow,
                    single_leg_arming ? "single_leg_gait4_direct_stable"
                                      : "gait4_direct_compute_only_stable");
              } else {
                ...
                if (reason != last_gait4_to_gait0_wait_reason) {
                  ...
                  std::cout << (single_leg_arming
                                    ? "[SINGLE-LEG-ALGO]"
                                    : "[LEGBOT-DDS][KEY3-PHASE]")
                            << " hold in LOCOMOTION_GAIT4_DIRECT before GAIT0_SHADOW"
                            << " reason=" << reason
                            << " publish=" << publish_tag
                            << " elapsed_in_phase=" << std::fixed
                            << std::setprecision(3) << phase_elapsed
          ...
          apply_gait0_blend_qdes_takeover();
          ...
          if (gait0_output_phase) {
            ...
            std::ostringstream monitor_warning;
            if (guard.num_tau_clamped > 0) {
              monitor_warning << "tau_clamped num_tau_clamped="
                              << guard.num_tau_clamped;
            } else if (gait0_monitor_qdes_clamped_count >=
                       kGait0BlendMonitorClampFrames) {
              monitor_warning << "qdes_clamped_consecutive count="
                              << gait0_monitor_qdes_clamped_count
                              << " num_qdes_clamped="
              ...
              monitor_warning << "dq_feedback_monitor_only max_abs_dq_feedback="
                              << monitor_dq_feedback
                              << " monitor_limit="
                              << kGait0BlendMonitorMaxAbsDqFeedback;
            } else if (!std::isfinite(monitor_tilt) ||
                       monitor_tilt > kGait0BlendMonitorMaxTilt) {
              monitor_warning << "tilt_monitor_only tilt=" << monitor_tilt
                              << " monitor_limit=" << kGait0BlendMonitorMaxTilt
                              << " hard_limit=max_tilt_rad_arg";
            } else if (gait0_monitor_entry_violation_count >=
                       kGait0BlendMonitorClampFrames) {
              monitor_warning << "entry_envelope_violation_consecutive count="
                              << gait0_monitor_entry_violation_count
                              << " joint="
                              << legbot::JointNameDds(
                                     gait0_entry_violation.joint)
            ...
            if (key3_phase == Key3Phase::kGait0Blend &&
                gait0_blend_entry_violation && now >= next_print) {
              std::cout << "[GAIT0-BLEND-MONITOR] warn reason="
                        ...
                        << gait0_monitor_entry_violation_count
                        << " abort_reason=none"
                        << " publish=GAIT0_BLEND_QDES_TAKEOVER"
                        << " requested_gait=" << requested_gait
            ...
            if (!monitor_warning.str().empty() && now >= next_print) {
              std::cout << "[GAIT0-MINIMAL] warn reason="
                        << monitor_warning.str()
      ...
      if (now >= next_print) {
        ...
        if (key3_phase == Key3Phase::kGait0Shadow) {
          PrintGait0ShadowRawDiagnostics(requested_gait, phase_elapsed,
                                         raw_command, snapshot, runner,
                                         controller);
        } else if (key3_phase == Key3Phase::kGait0Blend ||
            key3_phase == Key3Phase::kGait0Direct) {
          std::cout << "[GAIT0-MINIMAL]"
                    << " phase=" << Key3PhaseText(key3_phase)
                    << " elapsed=" << std::fixed << std::setprecision(3)
                    << phase_elapsed
                    << " publish=" << publish_tag
...
std::string RunControllerOutput(
  ...
  // LegBot gait0 minimal real bridge currently has no reliable base xy/velocity
  // estimator for DDS-only real tests. Use the existing cheater-state path,
  // which is filled every cycle from LowState: roll/pitch from IMU, yaw forced
  // to 0, base position fixed at nominal height, and base velocity/accel 0.
  // This does not change ConvexMPC/WBC output logic or release thresholds.
  robot_params.cheater_mode = 1;
  robot_params.use_rc = 0;
  ...
  std::cout << "[SINGLE-LEG-ALGO] configured probe_leg="
            << args.single_leg_probe_leg << "("
            << ModelLegName(args.single_leg_probe_leg) << ")\n";
  ...
  legbot::LegBotDDSCommandAdapter command_adapter;
  if (!real_output && args.foot_delta_probe_test) {
    return RunFootDeltaProbeComputeOnly(
        args, &runner, &spi_data, &spi_command, &vector_nav_data,
        &cheater_state, state_adapter, &command_adapter, stats, stats_t0);
  }
  if (real_output) {
    ...
    if (args.foot_delta_probe_test) {
      return RunFootDeltaProbeTest(
          args, &runner, &spi_data, &spi_command, &vector_nav_data,
          &cheater_state, state_adapter, lowcmd_pub, &command_adapter, stats,
          stats_t0);
    }
