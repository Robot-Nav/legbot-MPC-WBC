...
bool SafetyChecker<T>::checkSafeOrientation() {
  const auto& result = data->_stateEstimator->getResult();
  if (abs(result.rpy(0)) >= 0.5 || abs(result.rpy(1)) >= 0.5) {
    printf("Orientation safety check failed! rpy=[%.3f %.3f %.3f] "
           "vBody=[%.3f %.3f %.3f] vWorld=[%.3f %.3f %.3f]\n",
           result.rpy(0), result.rpy(1), result.rpy(2),
           result.vBody(0), result.vBody(1), result.vBody(2),
           result.vWorld(0), result.vWorld(1), result.vWorld(2));
    return false;
  } else {
    return true;
  }
