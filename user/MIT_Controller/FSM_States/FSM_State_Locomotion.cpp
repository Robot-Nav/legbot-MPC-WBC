    ...
    : FSM_State<T>(_controlFSMData, FSM_StateName::LOCOMOTION, "LOCOMOTION")
{
  if(_controlFSMData->_quadruped->_robotType == RobotType::MINI_CHEETAH ||
     _controlFSMData->_quadruped->_robotType == RobotType::LEGBOT){
...
void FSM_State_Locomotion<T>::onEnter() {
  ...
  if (this->_data->_quadruped->_robotType == RobotType::LEGBOT) {
    this->_data->userParameters->cmpc_gait = 4;
    this->_data->_gaitScheduler->gaitData._nextGait = GaitType::STAND;
    cMPCOld->initialize();
    cMPCOld->initializeStandingTargetForLegBot(*this->_data);
  } else {
    cMPCOld->initialize();
  ...
  if(this->_data->userParameters->use_wbc > 0.9){
    ...
    if (this->_data->_quadruped->_robotType == RobotType::LEGBOT) {
      ...
      if ((heightDebugCounter++ % 500) == 0) {
        const auto& se = this->_data->_stateEstimator->getResult();
        printf("[HEIGHT_DEBUG FSM] estimator_z=%.6f mpc_des_z=%.6f "
               "wbc_cmd_z=%.6f vWorld_z=%.6f contact=[%.2f %.2f %.2f %.2f]\n",
               (double)se.position[2], (double)cMPCOld->pBody_des[2],
               (double)_wbc_data->pBody_des[2], (double)se.vWorld[2],
               (double)_wbc_data->contact_state[0],
               (double)_wbc_data->contact_state[1],
               (double)_wbc_data->contact_state[2],
               (double)_wbc_data->contact_state[3]);
      }
    }
#endif
