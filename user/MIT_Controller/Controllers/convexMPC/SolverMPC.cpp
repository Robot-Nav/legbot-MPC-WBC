...
void solve_mpc(update_data_t* update, problem_setup* setup)
{
  ...
  if(std::isfinite(update->robot_mass) && update->robot_mass > 0.f) {
    rs.m = update->robot_mass;
  }
  bool body_inertia_valid = true;
  for(int i = 0; i < 9; ++i) {
    if(!std::isfinite(update->body_inertia[i])) {
      body_inertia_valid = false;
      break;
    }
  }
  const float body_inertia_trace =
      update->body_inertia[0] + update->body_inertia[4] +
      update->body_inertia[8];
  body_inertia_valid = body_inertia_valid && body_inertia_trace > 0.f;
  if(body_inertia_valid) {
    for(int r = 0; r < 3; ++r) {
      for(int c = 0; c < 3; ++c) {
        rs.I_body(r, c) = update->body_inertia[3 * r + c];
      }
    }
  }
  static bool printedRobotParams = false;
  if(!printedRobotParams) {
    printf("[MPC_ROBOT_PARAMS_DEBUG] dense_solver_mass=%.6f "
           "dense_solver_weight=%.6f body_inertia_trace=%.6f "
           "body_inertia_valid=%s gravity_z=-9.800000\n",
           (double)rs.m, (double)(rs.m * 9.8f),
           (double)(rs.I_body(0, 0) + rs.I_body(1, 1) + rs.I_body(2, 2)),
           body_inertia_valid ? "true" : "false");
    printedRobotParams = true;
  }
#ifdef K_PRINT_EVERYTHING
