...
namespace {
const char* getRobotDefaultsFile(RobotType robot) {
  switch (robot) {
    case RobotType::MINI_CHEETAH:
      return MINI_CHEETAH_DEFAULT_PARAMETERS;
    case RobotType::LEGBOT:
      return LEGBOT_DEFAULT_PARAMETERS;
    case RobotType::CHEETAH_3:
      return CHEETAH_3_DEFAULT_PARAMETERS;
    default:
      throw std::runtime_error("unknown robot defaults file");
  }
}

const char* getUserDefaultsFile(RobotType robot) {
  switch (robot) {
    case RobotType::LEGBOT:
      return "config/legbot-mit-ctrl-user-parameters.yaml";
    case RobotType::MINI_CHEETAH:
      return "config/mc-mit-ctrl-user-parameters.yaml";
    default:
      return nullptr;
  }
}
...
void SimulationBridge::run() {
  _robotParams.initializeFromYamlFile(std::string(THIS_COM "config") +
                                      getRobotDefaultsFile(_robot));
  if (_userParams) {
    const char* userDefaults = getUserDefaultsFile(_robot);
    if (userDefaults) {
      _userParams->initializeFromYamlFile(THIS_COM + std::string(userDefaults));
    }
  }
  ...
  try {
    ...
    snprintf(_sharedMemory().robotToSim.errorMessage,
             sizeof(_sharedMemory().robotToSim.errorMessage), "%s", e.what());
    throw e;
  }
