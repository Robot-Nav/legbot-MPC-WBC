...
void RobotRunner::init() {
  ...
  } else if (robotType == RobotType::LEGBOT) {
    _quadruped = buildLegBot<float>();
  } else {
  ...
  if (robotType == RobotType::LEGBOT) {
    _jpos_initializer = new JPosInitializer<float>(
        0.0, controlParameters->controller_dt,
        THIS_COM "config/legbot-initial_jpos_ctrl.yaml");
  } else {
    _jpos_initializer =
...
void RobotRunner::run() {
  ...
  ++count_ini;
  const bool runStartupDisablePulses = robotType != RobotType::LEGBOT;
  if (runStartupDisablePulses && count_ini < 10) {
    _legController->setEnabled(false);
  } else if (runStartupDisablePulses && 20 < count_ini && count_ini < 30) {
    _legController->setEnabled(false);
  } else if (runStartupDisablePulses && 40 < count_ini && count_ini < 50) {
    _legController->setEnabled(false);
  } else {
