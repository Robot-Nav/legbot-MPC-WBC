...
void printUsage() {
  printf(
      ...
      "\twhere robot-id:     3 for cheetah 3, m for mini-cheetah, v for LegBot\n"
...
int main_helper(int argc, char** argv, RobotController* ctrl) {
  ...
  if (argv[1][0] == '3') {
    ...
    gMasterConfig._robot = RobotType::LEGBOT;
  } else {
    printUsage();
    return EXIT_FAILURE;
  }
  ...
  const char* robotName = "Cheetah 3";
  if (gMasterConfig._robot == RobotType::MINI_CHEETAH) robotName = "Mini Cheetah";
  if (gMasterConfig._robot == RobotType::LEGBOT) robotName = "LegBot";
  printf("        Quadruped:  %s\n", robotName);
...
#ifdef linux
    if (gMasterConfig._robot == RobotType::LEGBOT) {
      printf("[ERROR] LegBot is currently supported in simulation mode only.\n");
      return EXIT_FAILURE;
