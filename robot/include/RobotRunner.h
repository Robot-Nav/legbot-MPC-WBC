...
class RobotRunner : public PeriodicTask {
 public:
  ...
  const LegController<float>* debugLegController() const {
    return _legController;
  }

  LegController<float>* debugMutableLegController() {
    return _legController;
  }

  const StateEstimate<float>& debugStateEstimate() const {
