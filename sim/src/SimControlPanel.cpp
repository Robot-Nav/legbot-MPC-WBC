...
void SimControlPanel::on_startButton_clicked() {
  ...
  } else if (ui->legbotButton->isChecked()) {
  ...
  if (!_simulationMode && robotType == RobotType::LEGBOT) {
    createErrorMessage("Error: LegBot is currently supported in simulation mode only");
    return;
  }
